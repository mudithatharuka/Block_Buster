	...
	if($celebrities){
		...
		if(mysqli_num_rows($celebrities) > 0){
			...
			while ($celebrity = mysqli_fetch_assoc($celebrities)) {
				...
				$celebrities_list.="<td><a href=\"modify-celebrity.php?cbr_id={$celebrity['cbr_id']}\">Edit/View</a></td>";
	...
	<main>
		...
		<div class="Search">
			<form action="celebrity.php" method="get">
				<p>
					<input type="text" name="search" placeholder="Type Celebrity Name, Celebrity ID or Admin Id and Hit Enter" value="<?php echo $search ?>" required autofocus>
				</p>
			</form>
		</div>
