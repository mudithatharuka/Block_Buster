	...
	if($tvseries){
		...
		if(mysqli_num_rows($tvseries) > 0){
			...
			while ($tvserie = mysqli_fetch_assoc($tvseries)) {
				...
				$tvseries_list.="<td><a href=\"modify-tvseries.php?series_id={$tvserie['series_id']}\">Edit/View</a></td>";
	...
	<main>
		...
		<div class="Search">
			<form action="tvseries.php" method="get">
				<p>
					<input type="text" name="search" placeholder="Type Movie Name, Movie ID or Admin Id and Hit Enter" value="<?php echo $search ?>" required autofocus>
				</p>
			</form>
		</div>
