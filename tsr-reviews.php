...
<?php
	...
	if(isset($_GET['search'])){
		...
		$query = "SELECT * FROM tsrreviews WHERE (review_id LIKE '%{$search}%' OR post_id LIKE '%{$search}%' OR user_id LIKE '%{$search}%') AND  is_deleted = 0 ORDER BY review_id DESC";
		// echo $query;
		// die();

	}else{
		$query = "SELECT * FROM tsrreviews WHERE is_deleted = 0 ORDER BY review_id DESC";
	...
	if($reviews){
		...
		if(mysqli_num_rows($reviews) > 0){
			...
			while ($review = mysqli_fetch_assoc($reviews)) {
				...
				$reviews_list.="<td><a href=\"delete-tsrreview.php?review_id={$review['review_id']}\"
...
<body>
	...
	<main>
		...
		<h1>TV Series Reviews
			<span>
				<a href="tsr-reviews.php"><button class="refr"><i class="fas fa-redo-alt"></i></button></a>
			</span>
		</h1>
		
		<div class="Search">
			<form action="tsr-reviews.php" method="get">
				<p>
					<input type="text" name="search" placeholder="Type Review ID,TV Series Id or User ID and Hit Enter" value="<?php echo $search ?>" required autofocus>
				</p>
			</form>
		</div>
