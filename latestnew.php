	...
	if($latestnews){
		...
		if(mysqli_num_rows($latestnews) > 0){
			...
			while ($latestnew = mysqli_fetch_assoc($latestnews)) {
				...
				$latestnews_list.="<td><a href=\"modify-latestnew.php?ltn_id={$latestnew['ltn_id']}\">Edit/View</a></td>";
	...
	<main>
		...
		<div class="Search">
			<form action="latestnew.php" method="get">
				<p>
					<input type="text" name="search" placeholder="Type Movie/Series Name, News ID or Admin Id and Hit Enter" value="<?php echo $search ?>" required autofocus>
				</p>
			</form>
		</div>
