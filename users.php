	...
	if($users){
		...
		if(mysqli_num_rows($users) > 0){
			...
			while ($user = mysqli_fetch_assoc($users)) {
				...
				$users_list.="<td><a href=\"modify-user.php?user_id={$user['user_id']}\">Edit</a></td>";
	...
	<main>
		...
		<div class="Search">
			<form action="users.php" method="get">
				<p>
					<input type="text" name="search" placeholder="Type User Name or User ID and Hit Enter" value="<?php echo $search ?>" required autofocus>
				</p>
			</form>
		</div>
